...
void delay_ms(volatile uint16_t delay)
{
	...
	//TIM4->PSC = (HAL_RCC_GetPCLK1Freq()*2)/1000000-1; //set Timer prescaler register
	TIM4->PSC = (HAL_RCC_GetPCLK1Freq())/1000000-1; //set Timer prescaler register
...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  SET_BIT(RCC->APB1ENR, RCC_APB1ENR_TIM4EN); // Delay Timer Initialization
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  btn_k0 = HAL_GPIO_ReadPin(BTN_K0_GPIO_Port, BTN_K0_Pin); //Read Pin 4 status

	  	  if(btn_k0 == 0) //if switch K0 Pressed
	  	  {
	  		  HAL_GPIO_WritePin(LED_D1_GPIO_Port, LED_D1_Pin, GPIO_PIN_RESET); // Reset Pin 9 (Led On)
	  		  delay_ms(2000000);
	  		  HAL_GPIO_WritePin(LED_D1_GPIO_Port, LED_D1_Pin, GPIO_PIN_SET); // Set Pin 9 (Led Off)
	  	  }
